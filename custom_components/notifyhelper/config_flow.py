...
class NotifyHelperConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        entry_names_set = {entry.data.get(CONF_ENTRY_NAME) for entry in current_entries}
        ios_devices_set = {
            device
        ...
        android_devices_set = {
            device
            for entry in current_entries
            for device in entry.data.get(CONF_ANDROID_DEVICES, [])
        }
        ...
        if user_input is not None:
            ...
            if user_input[CONF_ENTRY_NAME] in entry_names_set:
                ...
                input_ios_set = set(user_input[CONF_IOS_DEVICES])
                input_android_set = set(user_input[CONF_ANDROID_DEVICES])
            # 驗證是否至少選擇一個設備和設備是否重複
            if not input_ios_set and not input_android_set:
                errors["base"] = "no_device"
            elif (
                input_ios_set & ios_devices_set or input_android_set & android_devices_set
...
class NotifyHelperOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        entry_names_set = {entry.data.get(CONF_ENTRY_NAME) for entry in existing_entries}
        ios_devices_set = {
            device
